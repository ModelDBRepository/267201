 ...
 *
 *  Generated from NESTML at time: 2021-09-17 08:49:05.790908
**/
#ifndef MADEXP_PSC_ALPHA_REF
#define MADEXP_PSC_ALPHA_REF
...
**/
extern "C" inline int madexp_psc_alpha_ref_dynamics( double, const double y[], double f[], void* pnode );


/* BeginDocumentation
  Name: madexp_psc_alpha_ref.
...
*/
class madexp_psc_alpha_ref : public nest::ArchivingNode
{
public:
  /**
   * The constructor is only used to create the model prototype in the model manager.
  **/
  madexp_psc_alpha_ref();
  ...
  **/
  madexp_psc_alpha_ref(const madexp_psc_alpha_ref &);
...
private:
  ...
  **/
  ...
  friend class nest::RecordablesMap<madexp_psc_alpha_ref>;
  friend class nest::UniversalDataLogger<madexp_psc_alpha_ref>;
  ...
  **/
  struct Buffers_
  {
    Buffers_(madexp_psc_alpha_ref &);
    Buffers_(const Buffers_ &, madexp_psc_alpha_ref &);

    /**
     * Logger for all analog data
    **/
    nest::UniversalDataLogger<madexp_psc_alpha_ref> logger_;
  ...
  {
    return P_.E_0 + (P_.E_u - P_.E_0) * (1 - (std::max(S_.ode_state[State_::epsilon], 0.0)) / P_.epsilon_0);
  }
  ...
  static nest::RecordablesMap<madexp_psc_alpha_ref> recordablesMap_;
  friend int madexp_psc_alpha_ref_dynamics( double, const double y[], double f[], void* pnode );

}; /* neuron madexp_psc_alpha_ref */

inline nest::port madexp_psc_alpha_ref::send_test_event(nest::Node& target, nest::rport receptor_type, nest::synindex, bool)
{
...
inline nest::port madexp_psc_alpha_ref::handles_test_event(nest::SpikeEvent&, nest::port receptor_type)
{
    // You should usually not change the code in this function.
...
inline nest::port madexp_psc_alpha_ref::handles_test_event(nest::CurrentEvent&, nest::port receptor_type)
{
  // You should usually not change the code in this function.
  // It confirms to the connection management system that we are able
...
inline nest::port madexp_psc_alpha_ref::handles_test_event(nest::DataLoggingRequest& dlr, nest::port receptor_type)
{
  // You should usually not change the code in this function.
  // It confirms to the connection management system that we are able
...
inline void madexp_psc_alpha_ref::get_status(DictionaryDatum &__d) const
{
  // parameters
...
inline void madexp_psc_alpha_ref::set_status(const DictionaryDatum &__d)
{
  // parameters
...
#endif /* #ifndef MADEXP_PSC_ALPHA_REF */
